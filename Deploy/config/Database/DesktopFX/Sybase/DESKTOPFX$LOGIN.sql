IF EXISTS(SELECT * FROM sysobjects WHERE name = 'desktopfx$login' AND type = 'P')
    DROP PROCEDURE desktopfx$login
GO
...
BEGIN
    ...
    END
    ELSE BEGIN
        RAISERROR 99999, 'El usuario ya está login en la estación %s', @CURR_STATION
        RETURN
    END
    ...
    SELECT @FEC_ULT_LOG = REPLACE(CONVERT(VARCHAR,GETDATE(),25),' ',':')+'000'
...
sp_procxmode desktopfx$login, "anymode"
GO
