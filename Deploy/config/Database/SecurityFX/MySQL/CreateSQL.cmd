...
@if not defined MYSCON set MYSCON=--defaults-extra-file=CreateSQL.cnf
@if not defined MYSGRA set MYSGRA='ecuacc'@'localhost'

@rem mysql.exe %MYSCON% < SECURITYFX_Tables.sql
@rem @if %errorlevel% neq 0 goto errors

@rem mysql.exe %MYSCON% < SECURITYFX_Data.sql
@rem @if %errorlevel% neq 0 goto errors

@rem mysql.exe %MYSCON% < SECURITYFX_Grants.sql
@rem @if %errorlevel% neq 0 goto errors
