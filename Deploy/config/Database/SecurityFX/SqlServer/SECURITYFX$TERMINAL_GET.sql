IF OBJECT_ID(N'dbo.SECURITYFX$TERMINAL_GET', N'P') IS NOT NULL 
    DROP PROCEDURE dbo.SECURITYFX$TERMINAL_GET
GO

CREATE PROCEDURE dbo.SECURITYFX$TERMINAL_GET
--------------------------------------------------------------------------------
...
BEGIN
    ...
    IF (@@ROWCOUNT = 0) BEGIN
        RAISERROR('Terminal no está definido: %s', 16, 1, @TERMINAL_NAME);
        RETURN;
    END;
END
GO
