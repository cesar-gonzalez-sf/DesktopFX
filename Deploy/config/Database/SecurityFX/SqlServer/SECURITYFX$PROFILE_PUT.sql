IF OBJECT_ID(N'dbo.SECURITYFX$PROFILE_PUT', N'P') IS NOT NULL 
    DROP PROCEDURE dbo.SECURITYFX$PROFILE_PUT
GO

CREATE PROCEDURE dbo.SECURITYFX$PROFILE_PUT
--------------------------------------------------------------------------------
...
BEGIN
    ...
    IF (@CREATED = 0) BEGIN
        ...
        SET @AUDIT_MESSAGE = 'Perfil fue modificado: ' + @AUDIT_MESSAGE;
    END
    ELSE BEGIN
        SET @AUDIT_EVENT = 55;
        SET @AUDIT_MESSAGE = CAST(@PROFILE_CODE AS VARCHAR)
        SET @AUDIT_MESSAGE = 'Perfil fue creado: ' + @AUDIT_MESSAGE;
    END;
